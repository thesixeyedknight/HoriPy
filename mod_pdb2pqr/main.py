...
def print_pqr(args, pqr_lines, header_lines, missing_lines, is_cif):
    ...
    with open(args.output_pqr, "w") as outfile:
        ...
        # if header_lines: # SJ - commented out to avoid spamming the log
        #     _LOGGER.warning(
    ...
    for residue in biomolecule.residues:
        ...
        for atom in residue.atoms:
            if atom.alt_loc != "":
                ...
                #_LOGGER.warning(txt) #SJ - commented out to avoid spamming the log
        if multoccupancy:
            err = (
                f"Multiple occupancies found in {residue}. At least one of "
                "the instances is being ignored."
            )
            #_LOGGER.warning(err) #SJ - commented out to avoid spamming the log
...
def main_driver(args: argparse.Namespace, custom_pdblist=None): #SJ - added the custom_pdblist input
    ...
    if custom_pdblist is not None: #SJ - added this check so that it uses the custom_pdblist if provided
        pdblist = custom_pdblist
        is_cif = False
    else:
        pdblist, is_cif = io.get_molecule(args.input_path)#SJ - this function returns a list of PDB records (class defined in pdb.py) and a boolean indicating if the file is in CIF format
    if args.drop_water:
    ...
    if args.clean:
        ...
        try:
            results = non_trivial( #SJ - this function is the main driver for the PDB2PQR run
                args=args,
                biomolecule=biomolecule,
                ligand=ligand,
                definition=definition,
                is_cif=is_cif, #SJ - this is a boolean indicating if the file is in CIF format, if it is true, the code does not work
            )
        except ValueError as err:
            _LOGGER.critical(err)
            _LOGGER.critical("Giving up.")
            raise RuntimeError from err
    print_pqr(#SJ - this function prints the PQR file , I dont want to print pqr files
        args=args,
    ...
    if args.pdb_output:
        print_pdb(
            ...
            pdb_lines=io.print_biomolecule_atoms(
                biomolecule.atoms, chainflag=args.keep_chain, pdbfile=True #SJ - I'm interested in the biomolecule object, I am assuming this is the object that contains the atoms, charges and radii of the atoms and all the other information like residue and chain (basically all the information that is the pqr file)
            ),
            header_lines=results["header"],
            missing_lines=results["missed_residues"],
            is_cif=is_cif,
    ...
    return results, biomolecule #SJ - this function returns the missed residues, pKa dataframe and biomolecule object


def main(): #SJ - (not modified) this function is used when the script is run from the command line
...
def run_pdb2pqr(args: Sequence[str | PathLike]): #SJ - (not modified) this function is called by the __init__.py file

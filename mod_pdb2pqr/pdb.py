...
class ATOM:
    """
    Unified ATOM class to handle both PDB and CIF formats.
    
    Parses atomic information from both file types and provides consistent
    attributes.
    """

    def __init__(self, line, filetype):
        """
        Initialize by parsing either PDB or CIF format line.

        :param line:  Line containing atom data.
        :type line:  str or list (for CIF)
        :param is_cif:  Flag indicating if the input is CIF format.
        :type is_cif:  bool
        """
        record = line[0:5].strip()
        if record != 'ATOM':
            raise ValueError(f'Expected ATOM record but got {record}')
        self.original_text = line.rstrip("\r\n")
        ...
        if filetype == "cif":
            ...
            # PDB line parsing (similar to original ATOM class).
            self.serial = int(line[6:11].strip())
            self.name = line[12:16].strip()
            self.alt_loc = line[16].strip()
            self.res_name = line[17:20].strip()
    ...
    def __repr__(self):
        ...
        return (
            f"UnifiedATOM(serial={self.serial}, name='{self.name}', res_name='{self.res_name}', "
            f"chain_id='{self.chain_id}', res_seq={self.res_seq}, x={self.x}, y={self.y}, z={self.z}, "
            f"occupancy={self.occupancy}, temp_factor={self.temp_factor}, element='{self.element}', charge='{self.charge}')"
        )
